...
use crate::ui::text::document::ParsedDocument;
use crate::ui::text::node::{
...
fn parse_paragraph(paragraph: &mut Paragraph, node: &Rc<Node>) {
    fn push_merged(
        ...
        if let Some(new_mark) = new_mark {
            ...
            let merged = if let Some(last) = node.marks.last_mut() {
                if last.0.start == 0 && last.0.end == len {
                    last.1.merge(new_mark.clone());
                    true
                } else {
                    false
                }
