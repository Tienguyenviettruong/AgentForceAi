...
impl TextViewState {
    ...
    fn new(format: TextViewFormat, text: &str, cx: &mut Context<Self>) -> Self {
        ...
        let (parsed_content, parsed_error) =
            match parse_content(format, ParsedContent::default(), &initial_options) {
                Ok(content) => (content, None),
                Err(err) => (ParsedContent::default(), Some(err)),
            };
    ...
    if options.append {
        if let Some(last_block) = content.document.blocks.pop() {
            if let Some(span) = last_block.span() {
                ...
                source = format!("{}{}", last_source, options.pending_text);
            } else {
